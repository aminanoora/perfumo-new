...
function updateCartSummary(data) {
  ...
  const giftWrap = document.getElementById("cartGiftWrap");

  const grandTotal = document.getElementById("cartGrandTotal");

  if (subtotal) {
    subtotal.textContent = "₹" + Number(data.subtotal).toLocaleString("en-IN");
  }
  ...
  if (shipping) {
    shipping.textContent =
      Number(data.shipping) === 0
        ? "Free"
        : "₹" + Number(data.shipping).toLocaleString("en-IN");
  }

  if (giftWrap) {
    giftWrap.textContent =
      "₹" + Number(data.giftWrapAmount).toLocaleString("en-IN");
  }
    ...
    try {
      ...
      if (itemTotal) {
        itemTotal.textContent =
          "₹" + Number(data.itemTotal).toLocaleString("en-IN");
      }
    ...
    try {
      ...
      if (!data.success) {
        Swal.fire({
          ...
          title: data.message || "Unable to remove item",
        });
